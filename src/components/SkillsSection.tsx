      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/10 pb-6 relative z-10">
        <div>
          ...
          <h2 className="text-xl sm:text-3xl md:text-4xl font-black font-montserrat tracking-tight text-white uppercase flex flex-wrap items-baseline gap-2 sm:gap-3">
            <span className="text-xl sm:text-3xl md:text-4xl">Habilidades</span>
            <span className="font-script text-2xl sm:text-3xl md:text-4xl text-[#00d2ff] font-normal normal-case neon-glow-blue">
              Profesionales
            </span>
          </h2>
        </div>
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-5">
        {filteredSkills.map((skill: SkillItem, idx: number) => {
          return (
            <div
              key={idx}
              className="p-5 rounded-2xl bg-gradient-to-br from-white/[0.05] to-white/[0.015] border border-white/10 hover:border-cyan-400/40 transition-all duration-300 shadow-lg group"
            >
              {/* Skill Header */}
              <div className="flex items-center justify-between gap-3 mb-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-zinc-400 font-montserrat uppercase">
                      {skill.category}
                    </span>
                  </div>
                </div>
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-white/[0.05] to-white/[0.015] border border-white/10 shadow-lg">
          ...
          <div className="space-y-3">
            {personalInfo.languages.map((lang, lIdx) => (
              <div key={lIdx} className="flex items-center justify-between p-2.5 rounded-xl bg-black/40 border border-white/5">
                <span className="font-montserrat text-xs sm:text-sm font-semibold text-white">
                  {lang.name}
                </span>
                <span className="text-xs font-oswald text-cyan-300 bg-cyan-950/40 px-2.5 py-0.5 rounded-md border border-cyan-500/30">
                  {lang.level}
                </span>
              </div>
            ))}
